 ...
 */
public class ProgramGeekBFSDFSCloneGraph {
  ...
  // source is 1, then BFS 1->2->4->3->5	
	public void bfs(Node source) {
	  ...
	  //  |  | 
	  //  4--3--5
	  // source is 1, then DFS ,  1,4,3,5,2, from last one can not 
	 ...
	 *  Create Map<A,a> and queue<Node>
	 *. BFS loop A.neighors to B, 
	 *  check if  Map.get(B)
	 *  if it is not in Map, Map.get(B)==null 
	    q.add(B) keep BFS going
	    clone_b = new Node(B.val)
	    map.put(B,clone_b)
	 *. add clone_b to the neighbors of cloneA
	 */
	...
	public static void main(String[] args) {
		...
		ProgramGeekBFSDFSCloneGraph graph = new ProgramGeekBFSDFSCloneGraph(); 
	         Node source = graph.buildGraph(); 
	        System.out.println("BFS traversal of a graph before cloning"); 
	        graph.bfs(source); 
	        
	        System.out.println("DFS traversal of a graph before cloning"); 
	        graph.dfs(source); 
